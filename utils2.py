...
def load_images_and_resize(dataset, new_size=[120, 160], train=True, force_recalc=False, return_entire_data=False):
    ...
    for i in range(n_clip):
        clip_path = '%s/frames/%s/' % (img_dir, str(i+1).zfill(2))
        print(clip_path)
        # image
        img_files = sorted(glob.glob(clip_path + '*.jpg'))
...
def load_images_single_clip(dataset, clip_idx, indices, train=True):
    ...
    print('number of images: ', n_images)
    #
    clip_path = '%s/frames/%s/' % (img_dir, str(clip_idx+1).zfill(3))
    print(clip_path)
    # image
    img_files = sorted(glob.glob(clip_path + '*.jpg'))
...
def split_cubes(dataset, clip_idx, cube_size, training_set=True, force_recalc=False, dist_thresh=None):
    ...
    if os.path.isfile(saved_cube_file) and not force_recalc:
        ...
        seqrot = loader['seqrot']
        return cubes, mapping, seqrot
    ...
    np_label = np.zeros((h_grid * w_grid * d_grid, 4, 4), dtype=int)
    np_rot = np.zeros((h_grid * w_grid * d_grid, 4, 1), dtype=int)
    seqrot = np.zeros((h_grid * w_grid * d_grid, 4, 5), dtype=int)
#     print(cubes.shape, image_data.shape)
    for j in range(d_grid): # clip number(dim 축)
        for k in range(h_grid):
            for l in range(w_grid):
                ##frame 4장 섞기
                indice_data = list(enumerate(image_data[j:j+4]))
                random.shuffle(indice_data)
                sequence, new_image_data = zip(*indice_data)
                new_image_data = np.asarray(new_image_data)
                for x in list(range(0,4)):
                    y = sequence[x]
                    np_label[j*h_grid*w_grid+k*w_grid+l][y][x] = 1
                ##로테이션
                rotation = random.randrange(0, 4)
                np_rot[j*h_grid*w_grid+k*w_grid+l][rotation] = 1
                #seq+rot 라벨
                seqrot[j*h_grid*w_grid+k*w_grid+l] = np.concatenate((np_label[j*h_grid*w_grid+k*w_grid+l], np_rot[j*h_grid*w_grid+k*w_grid+l]), axis=1)
                #shuffle, rotate된 10x10x4 cube 생성
                a=new_image_data[:,k*cube_size[0]:(k+1)*cube_size[0],l*cube_size[1]:(l+1)*cube_size[1]]
                a=np.rot90(a, k=rotation, axes=(1,2))
                cubes[j*h_grid*w_grid+k*w_grid+l]=np.moveaxis(a, 0, -1)
                #spatial location 라벨
    ...
    np.savez_compressed(saved_cube_file, data=cubes, mapping=mapping, seqrot=seqrot)
    print('new shape2:', cubes.shape, image_data.shape)
    return cubes, mapping, seqrot
...
def load_all_cubes_in_set(dataset, h, w, cube_size, training_set=True):
    ...
    seqrot = np.zeros((n_cube_in_set, 4, 5), dtype=int)
    idx = 0
    for clip_idx in range(n_clip):
        tmp_cubes, tmp_mapping, tmp_seqrot = split_cubes(dataset, clip_idx, cube_size, training_set=training_set)
        assert len(tmp_cubes) == len(tmp_mapping)
        cubes[idx:idx+len(tmp_cubes)] = tmp_cubes
        mapping[idx:idx+len(tmp_mapping)] = tmp_mapping
        seqrot[idx:idx+len(tmp_seqrot)] = tmp_seqrot
        idx += len(tmp_mapping)
    ...
    seqrot = np.delete(seqrot, idx, axis=0)
    print(cubes.shape, mapping.shape, seqrot.shape)
    #
    return cubes, mapping, seqrot
...
def count_sequence_n_frame(dataset, test=True):
    ...
    print("num=", sequence_n_frame)
    for i in range(len(sequence_n_frame)):
        clip_path = '%s/frames/%s/' % (dataset['path_test' if test else 'path_train'], str(i+1).zfill(2))
